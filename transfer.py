...
from model.transferability.data.dataloader import get_dataloader_splits
from model.utils.metrics import evaluate, average_folds_results, save_results
from model.transferability.modeling.adapters import LinearProbe, ClipAdapter, ZeroShot, TipAdapter
...
def process(args):
    ...
    for iFold in range(args.folds):
        ...
        model = Model(vision_type=args.architecture, bert_type='./Bio_ClinicalBERT', from_checkpoint=args.load_weights, weights_path=args.weights_path,
...
def main():
    ...
    parser.add_argument('--save_model', default=True, type=lambda x: (str(x).lower() == 'true'))
    ...
    parser.add_argument('--weights_path', default='local_data/results/pretraining/checkpoint.pth', help='')
    ...
    parser.add_argument('--shots_train', default="100%", type=lambda x: (str(x)))
    parser.add_argument('--shots_val', default="0%", type=lambda x: (str(x)))
    parser.add_argument('--shots_test', default="0%", type=lambda x: (str(x)))
    ...
    parser.add_argument('--num_workers', default=3, type=int, help='workers number for DataLoader')
